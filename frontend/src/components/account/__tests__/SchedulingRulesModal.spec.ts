...
describe('SchedulingRulesModal', () => {
  it('saves the selected lowest-cost rule, disables the legacy overlay, and updates the probe interval', async () => {
    ...
    api.getProbeSettings.mockResolvedValue({ enabled: true, interval_minutes: 30 })
    ...
    expect(api.updateSuperPriority).toHaveBeenCalledWith(expect.objectContaining({ base_strategy: 'lowest_cost' }))
    expect(api.updateProbeSettings).toHaveBeenCalledWith({ enabled: true, interval_minutes: 5 })
