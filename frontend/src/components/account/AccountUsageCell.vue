      ...
      <div v-else-if="usageInfo" class="space-y-1">
        ...
        <template v-else>
          ...
          <UsageProgressBar
            ...
            color="indigo"
          />
          <div v-if="grokPrepaidMoneyLine" class="flex flex-wrap items-center gap-1 text-[10px] text-gray-500 dark:text-gray-400">
            <span class="rounded bg-emerald-50 px-1 py-0.5 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300" :title="t('admin.accounts.usageWindow.grokPrepaid')">
...
const grokPrepaidMoneyLine = computed(() => {
  const billing = grokBilling.value
  if (!billing || billing.prepaid_balance == null || !Number.isFinite(billing.prepaid_balance)) return null
  const used = billing.monthly_used ?? (billing.used_cents != null ? billing.used_cents / 100 : 0)
  const limit = billing.monthly_limit ?? (billing.monthly_limit_cents != null ? billing.monthly_limit_cents / 100 : 0)
  return {
