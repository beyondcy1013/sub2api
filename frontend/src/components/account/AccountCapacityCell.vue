...
const windowCostClass = computed(() => {
  ...
  if (current >= limit + reserve) return 'text-red-600 dark:text-red-400'
  if (current >= limit) return 'text-orange-600 dark:text-orange-400'
  if (current >= limit * 0.8) return 'text-amber-600 dark:text-amber-400'
...
const sessionLimitClass = computed(() => {
  ...
  if (current >= max) return 'text-red-600 dark:text-red-400'
  if (current >= max * 0.8) return 'text-amber-600 dark:text-amber-400'
  return 'text-emerald-600 dark:text-emerald-400'
})
...
const rpmClass = computed(() => {
  ...
  if (rpmStrategy.value === 'tiered') {
    if (current >= base + buffer) return 'text-red-600 dark:text-red-400'
    if (current >= base) return 'text-orange-600 dark:text-orange-400'
  } else {
    if (current >= base) return 'text-orange-600 dark:text-orange-400'
  }
  if (current >= base * 0.8) return 'text-amber-600 dark:text-amber-400'
  return 'text-emerald-600 dark:text-emerald-400'
})
