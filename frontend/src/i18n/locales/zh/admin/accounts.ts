export default {
    accounts: {
      ...
      restoreDeletedFailed: '恢复删除态账号失败',
      ...
      enhancedImportClearHint: '解析与增强导入相同的 JSON 文件或混杂消息，并将数据库中匹配的账号移入可恢复的已删除暂存；代理不会被删除。',
      enhancedImportClearing: '清除中...',
      enhancedImportClearNoAccounts: '当前内容中没有可清除的账号',
      enhancedImportClearConfirm: '确定按当前内容匹配并清除账号吗？共解析到 {count} 个账号。匹配账号将移入可恢复的已删除暂存，代理不会被删除。',
      enhancedImportClearSuccess: '清除完成：{account_cleared} 个账号已移入已删除暂存。',
      enhancedImportClearCompletedWithErrors: '部分清除完成：已清除 {account_cleared}，未找到 {account_not_found}，匹配歧义 {account_ambiguous}，失败 {account_failed}。',
      ...
      schedulingRules: {
        ...
        helpLiveness: '存活检测：仅检测 Key 类型账号；一次失败标记为待确认，连续失败达到阈值后排除，后续检测成功会自动恢复。OAuth 等其他类型不参与。',
        strategy: '调度策略',
        default: '默认调度',
        lowestCost: '低价优先',
        liveness: '连接存活检测',
        ...
        livenessIncludeUnschedulable: '包含已暂停账号',
        livenessIncludeUnschedulableHint: '开启后，手动暂停调度的 Key 类型账号也会参与连接存活检测；默认不检测。',
        livenessHint: '仅在低价优先模式检测 Key 类型账号；OAuth 等其他类型不参与。失败一次先标记待确认，达到阈值后排除，后续成功会自动恢复。',
        runtimeLabel: '循环测试',
        runtimeLoading: '正在读取状态',
        runtimeUnavailable: '状态暂不可用',
        runtimeRunning: '正在检测连接存活',
      ...
      accountDeleted: '账号已移入删除态',
      rateLimitCleared: '速率限制已清除',
      setupToken: 'Setup Token',
      apiKey: 'API Key',
      deleteConfirm: "确定要将账号 '{name}' 移入删除态吗？账号数据会完整保留并可随时恢复，但不再参与请求调度和各类自动扫描。",
