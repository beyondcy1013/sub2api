...
- New-account form initialization and reset select the last available proxy and the first available group. Late-arriving proxy/group candidates fill only empty selections, never overwrite an operator choice, and empty candidate lists remain unassigned.
...
Build, atomic binary replacement, live verification, fork push, cleanup, and rollback must follow [UPGRADE_RUNBOOK.md](UPGRADE_RUNBOOK.md). Restart only `sub2api.service`; never use a shorter direct-overwrite deployment command.
