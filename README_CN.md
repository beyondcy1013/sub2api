...
> **它不是简单的请求转发器，而是一套能长期运行、扛住并发、看清成本、快速恢复的 AI API 运营中枢。**
> 当账号池越来越大、流量越来越复杂，稳定调度和高效运维比“能转发”本身更重要。

| 增强能力 | 带来的实际价值 |
|------|------|
| **低成本智能调度** | 按真实调度倍率选择更优账号，结合周期存活探测自动避开新鲜失活节点，并在账号恢复后重新纳入调度 |
| **粘性会话并发溢出** | 历史绑定账号满载时自动切换到同组可用账号，避免单个热点账号拖住整批请求；严格续传亲和性保持不变 |
| **额度与成本全景视图** | 独立展示 5h/7d 请求数、Token、利用率、重置时间和上下游成本，支持单账号查询与并发批量刷新 |
| **更强的账号运营工具** | 提供暂存区、可恢复回收站、变更追踪式批量编辑、混合消息 JSON 增强导入和紧凑高密度账号表格 |
| **自动恢复与定时动作** | 可定时启用并恢复、暂停调度；定时测试成功后可自动清理异常状态并重新启用调度 |
| **一套源码，双环境隔离** | 同一份已验证产物同时服务 main/free profile，同时严格隔离数据库、Redis、配置、端口和运行生命周期 |

**账号越多、流量越复杂，这个增强版越能体现价值。** 适合重视稳定性、成本透明度和可恢复运维的个人与团队。
...
> [!WARNING]
> **本定制增强版禁止使用 WebUI 二进制更新器。** 它会安装上游原版二进制并覆盖本地增强功能。请按照 [统一升级运行手册](docs/UPGRADE_RUNBOOK.md)，通过源码合并、完整验证和统一构建完成升级。
