...
func TestOpenAIGatewayService_SelectAccountWithScheduler_SessionStickyBusySpillsOverEvenWhenHealthEscapeDisabled(t *testing.T) {
	ctx := context.Background()
	groupID := int64(10100)
	accounts := []Account{
		{
	...
	require.Equal(t, openAIAccountScheduleLayerLoadBalance, decision.Layer)
	require.False(t, decision.StickySessionHit)
	require.Equal(t, int64(21001), cache.sessionBindings["openai:session_hash_sticky_busy"])
	if selection.ReleaseFunc != nil {
		selection.ReleaseFunc()
	}
}
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_HealthEscapeDisabledStillSpillsOnConcurrency(t *testing.T) {
	ctx := context.Background()
	groupID := int64(10104)
	accounts := []Account{
