...
func TestFilterGrokFreeQuotaAccountsOnlyBlocksExplicitFreeOAuth(t *testing.T) {
	...
	require.Equal(t, []int64{1, 2, 3, 4}, grokQuotaAccountIDs(filtered), "miss fails open on hot path")
	...
	filtered = scheduler.filterGrokFreeQuotaAccounts(context.Background(), accounts)
	require.Equal(t, []int64{2, 3, 4}, grokQuotaAccountIDs(filtered), "paid and unknown fail-open; API-key free marker is not gated")
...
func TestFilterGrokFreeQuotaAccountsStatsFailureFailsOpen(t *testing.T) {
	...
	filtered := scheduler.filterGrokFreeQuotaAccounts(context.Background(), accounts)
	require.Equal(t, []int64{1}, grokQuotaAccountIDs(filtered))
	require.Eventually(t, func() bool {
		repo.mu.Lock()
		defer repo.mu.Unlock()
		return repo.calls >= 1
...
func TestFilterGrokFreeQuotaAccountsUnknownTierFailOpen(t *testing.T) {
	...
	filtered := scheduler.filterGrokFreeQuotaAccounts(context.Background(), accounts)
	require.Equal(t, []int64{1, 2, 3}, grokQuotaAccountIDs(filtered))
...
func TestFilterGrokFreeQuotaAccountsRecoversAfterRollingUsageFalls(t *testing.T) {
	...
	require.Equal(t, []int64{1}, grokQuotaAccountIDs(scheduler.filterGrokFreeQuotaAccounts(context.Background(), accounts)))
	require.Eventually(t, func() bool {
		filtered := scheduler.filterGrokFreeQuotaAccounts(context.Background(), accounts)
...
func TestOpenAIAccountSchedulerLoadBalanceAppliesGrokFreeQuotaGate(t *testing.T) {
	...
	require.Eventually(t, func() bool {
		filtered := scheduler.filterGrokFreeQuotaAccounts(context.Background(), accounts)
		return len(grokQuotaAccountIDs(filtered)) == 1 && grokQuotaAccountIDs(filtered)[0] == 2
	}, 2*time.Second, 10*time.Millisecond)
...
func grokQuotaAccountIDs(accounts []Account) []int64 {
