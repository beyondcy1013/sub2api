...
func TestOpenAIImagesJSONKeepalive_FastErrorPreservesStatus(t *testing.T) {
	...
	require.Equal(t, "【sub2freeApi限制】 invalid size", gjson.Get(rec.Body.String(), "error.message").String())
}
...
func TestOpenAIImagesJSONKeepalive_LateErrorRemainsJSON(t *testing.T) {
	...
	require.Equal(t, "【sub2freeApi限制】 request rejected", gjson.Get(rec.Body.String(), "error.message").String())
}
