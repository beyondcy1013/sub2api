...
func (r *accountRepository) updateAccount(ctx context.Context, account *service.Account, explicitProbeEnabled, explicitRateSyncEnabled *bool, explicitRateMultiplier *float64) error {
	if account == nil {
		return nil
	}
...
func (r *accountRepository) updateLockedAccount(ctx context.Context, client *dbent.Client, account *service.Account, explicitProbeEnabled, explicitRateSyncEnabled *bool, explicitRateMultiplier *float64) (*dbent.Account, error) {
...
func (r *accountRepository) UpdateUpstreamBillingProbeSnapshot(
	...
	if dbent.TxFromContext(ctx) == nil {
		...
		if errors.Is(err, dbent.ErrTxStarted) {
			return r.updateUpstreamBillingProbeSnapshotInTx(ctx, account, snapshot, firstRateMultiplier(rateMultipliers))
		}
		if err != nil {
			return err
		}
		...
		if err := r.updateUpstreamBillingProbeSnapshotInTx(dbent.NewTxContext(ctx, tx), account, snapshot, firstRateMultiplier(rateMultipliers)); err != nil {
			return err
		}
		if err := tx.Commit(); err != nil {
			return err
...
func firstRateMultiplier(values []*float64) *float64 {
	if len(values) == 0 {
		return nil
	}
	return values[0]
...
func (r *accountRepository) updateUpstreamBillingProbeSnapshotInTx(
	...
	resolvedRate, hasResolvedRate := service.ResolvedSchedulingRateFromProbeSnapshot(snapshot)
	var updateSQL string
	var args []any
	if explicitRateMultiplier != nil || !hasResolvedRate {
		updateSQL = `
		UPDATE accounts
		SET
	...
	`
		var rateArg any
		if explicitRateMultiplier != nil && snapshot.Status == service.UpstreamBillingProbeStatusOK {
			rateArg = *explicitRateMultiplier
		}
		args = []any{string(payload), account.ID, account.Platform, account.Type, string(credentials), proxyID, string(expectedSnapshotJSON), string(expectedEnabledJSON), string(expectedRateSyncEnabledJSON), rateArg}
	} else {
		updateSQL = `
		UPDATE accounts
		SET
	...
	`
		...
		if snapshot.Status == service.UpstreamBillingProbeStatusOK {
			rateArg = resolvedRate
		}
		args = []any{string(payload), account.ID, account.Platform, account.Type, string(credentials), proxyID, string(expectedSnapshotJSON), string(expectedEnabledJSON), rateArg}
	}
