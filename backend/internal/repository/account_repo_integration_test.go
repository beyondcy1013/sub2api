...
func (s *AccountRepoSuite) TestListWithFilters() {
	...
	for _, tt := range tests {
		s.Run(tt.name, func() {
			...
			accounts, page, err := repo.ListWithFilters(ctx, pagination.PaginationParams{Page: 1, PageSize: 10}, tt.platform, tt.accType, tt.status, tt.search, tt.groupID, tt.privacyMode, false)
			s.Require().NoError(err)
...
func (s *AccountRepoSuite) TestPreload_And_VirtualFields() {
	...
	accounts, page, err := s.repo.ListWithFilters(s.ctx, pagination.PaginationParams{Page: 1, PageSize: 10}, "", "", "", "acc", 0, "", false)
