...
func WithSource(message string) string {
	...
	return WithGatewaySource(message)
}

// WithGatewaySource marks a client-visible error as generated by this gateway.
// Unlike WithSource, it is explicit and applies to both deployment profiles.
func WithGatewaySource(message string) string {
	message = strings.TrimSpace(message)
	if message == "" {
		return sourceMarker
	}
