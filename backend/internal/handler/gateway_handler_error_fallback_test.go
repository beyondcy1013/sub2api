...
func TestGatewayEnsureForwardErrorResponse_MainProfileIncludesSource(t *testing.T) {
	...
	require.Equal(t, "Upstream request failed (source: sub2api)", errorObj["message"])
	require.Equal(t, "sub2api", errorObj["source"])
}
