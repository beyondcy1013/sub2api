...
func TestSchedulingRulesRefreshRunsLivenessAndBillingRefreshers(t *testing.T) {
	...
	livenessData, ok := data["liveness"].(map[string]any)
	require.True(t, ok)
	billingData, ok := data["upstream_billing"].(map[string]any)
	require.True(t, ok)
	require.Equal(t, float64(3), livenessData["checked"])
	require.Equal(t, float64(2), billingData["checked"])
}
...
func TestSuperPrioritySettingsHandlerReturnsLivenessRuntimeStatus(t *testing.T) {
	...
	runtime, ok := data["liveness_runtime"].(map[string]any)
	require.True(t, ok)
	require.Equal(t, true, runtime["enabled"])
	require.Equal(t, nextRunAt.Format(time.RFC3339), runtime["next_run_at"])
	lastRun, ok := runtime["last_run"].(map[string]any)
	require.True(t, ok)
	require.Equal(t, "scheduled", lastRun["trigger"])
	result, ok := lastRun["result"].(map[string]any)
	require.True(t, ok)
	require.Equal(t, float64(1), result["skipped"])
}
...
func TestSchedulingRulesRuntimeReturnsBothTaskStatuses(t *testing.T) {
	...
	livenessData, ok := data["liveness"].(map[string]any)
	require.True(t, ok)
	livenessEnabled, ok := livenessData["enabled"].(bool)
	require.True(t, ok)
	billingData, ok := data["upstream_billing"].(map[string]any)
	require.True(t, ok)
	billingEnabled, ok := billingData["enabled"].(bool)
	require.True(t, ok)
	require.True(t, livenessEnabled)
	require.True(t, billingEnabled)
}
