...
func TestOpenAIHandleStreamingAwareError_JSONEscaping(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, clienterror.WithSource(clienterror.Prefix(tt.errType, tt.message)), errorObj["message"])
...
func TestOpenAIHandleStreamingAwareError_NonStreaming(t *testing.T) {
	...
	assert.Equal(t, "【上游错误】 test error (source: sub2freeApi)", errorObj["message"])
...
func TestOpenAIEnsureForwardErrorResponse_ImageJSONKeepaliveWritesSingleJSONFallback(t *testing.T) {
	...
	require.Equal(t, "【上游错误】 Upstream request failed (source: sub2freeApi)", gjson.Get(w.Body.String(), "error.message").String())
}
...
func TestOpenAIResponses_APIKeyPassthroughPool5xxRetriesThenExhaustsMaxSwitches(t *testing.T) {
	...
	require.Equal(t, "【上游错误】 Upstream service temporarily unavailable (source: sub2freeApi)", gjson.GetBytes(rec.Body.Bytes(), "error.message").String())
}
