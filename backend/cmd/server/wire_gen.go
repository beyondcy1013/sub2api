...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	relayFailureBudgetCache := repository.NewRelayFailureBudgetCache(redisClient)
	geminiTokenCache := repository.NewGeminiTokenCache(redisClient)
	compositeTokenCacheInvalidator := service.NewCompositeTokenCacheInvalidator(geminiTokenCache)
	rateLimitService := service.ProvideRateLimitService(accountRepository, usageLogRepository, configConfig, geminiQuotaService, tempUnschedCache, timeoutCounterCache, openAI403CounterCache, relayFailureBudgetCache, settingService, compositeTokenCacheInvalidator)
	...
	superPriorityService := service.ProvideSuperPriorityService(accountRepository, configConfig)
	...
	superPriorityRunner := service.ProvideSuperPriorityRunner(superPriorityService, accountTestService, accountRepository)
	...
	stickySessionAdminStore := repository.ProvideStickySessionAdminStore(redisClient, configConfig)
	accountHandler := admin.ProvideAccountHandler(adminService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, grokOAuthService, rateLimitService, accountUsageService, accountTestService, concurrencyService, crsSyncService, sessionLimitCache, rpmCache, compositeTokenCacheInvalidator, grokQuotaService, stickySessionAdminStore)
	...
	settingHandler := handler.ProvideAdminSettingHandler(settingService, emailService, turnstileService, opsService, paymentConfigService, paymentService, userAttributeService, notificationEmailService, totpService, userService, superPriorityService)
	...
	scheduledAccountActionRepository := repository.NewScheduledAccountActionRepository(db)
	scheduledAccountActionService := service.ProvideScheduledAccountActionService(scheduledAccountActionRepository, adminService, rateLimitService)
	scheduledAccountActionHandler := admin.NewScheduledAccountActionHandler(scheduledAccountActionService)
	...
	adminHandlers := handler.ProvideAdminHandlers(superPriorityService, superPriorityRunner, dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, scheduledAccountActionHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, promptAdminHandler, paymentHandler, affiliateHandler, complianceHandler, auditLogHandler, upstreamBillingProbeService, ollamaCloudUsageService)
