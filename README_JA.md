...
> **単なるリクエスト転送ではなく、継続稼働、同時実行負荷、コストの可視化、迅速な復旧まで担う AI API 運用基盤です。**
> アカウントプールとトラフィックが大きくなるほど、転送機能だけでなく、安定したスケジューリングと効率的な運用が重要になります。

| 強化機能 | 運用上の価値 |
|---------|-------------|
| **最低コストのインテリジェントスケジューリング** | 実際のスケジューリング倍率で最適なアカウントを選択し、定期的な死活監視で新たに停止したノードを回避、復旧後は自動的に再参加させます |
| **スティッキーセッションの同時実行オーバーフロー** | 過去にバインドされたアカウントが満杯の場合、同じグループの利用可能なアカウントへ振り分け、厳密な継続リクエストの親和性を維持したままボトルネックを防ぎます |
| **クォータとコストの完全な可視化** | 5h/7d のリクエスト数、Token、使用率、リセット時刻、上流・ユーザーコストを個別表示し、単体照会と同時実行数を制限した一括更新に対応します |
| **強力なアカウント運用ツール** | ステージング、復元可能なゴミ箱、変更追跡付き一括編集、混在メッセージからの JSON インポート、高密度なアカウントテーブルを提供します |
| **自動復旧と予約アクション** | 有効化・復旧やスケジューリング停止を予約でき、定期テスト成功時には異常状態の解消とスケジューリング再開を自動化できます |
| **単一ソースと分離されたデュアルプロファイル** | 同一の検証済み成果物を main/free で使用しながら、データベース、Redis、設定、ポート、サービスのライフサイクルを厳密に分離します |

**アカウント数とトラフィックの複雑さが増すほど、この強化版の価値は高まります。** 安定性、コストの透明性、復旧可能な運用を重視する利用者向けです。
...
> [!WARNING]
> **このカスタマイズ版では WebUI のバイナリアップデーターを使用しないでください。** 上流版のバイナリでローカル拡張が上書きされます。[統合アップグレード手順](docs/UPGRADE_RUNBOOK.md)に従い、ソースのマージ、完全な検証、共有成果物のビルドを行ってください。
