...
WEBCLX_TOOLCHAIN="/home/codes/webClx/.android-toolchain"
...
if [[ ! -s "${GRADLE_ARCHIVE}" && -s "${WEBCLX_TOOLCHAIN}/gradle-${GRADLE_VERSION}-bin.zip" ]]; then
    cp -a "${WEBCLX_TOOLCHAIN}/gradle-${GRADLE_VERSION}-bin.zip" "${GRADLE_ARCHIVE}"
fi
...
"${GRADLE_HOME}/bin/gradle" \
    ...
    "-Psub2apiVersion=${client_version}" \
    "-Psub2apiVersionCode=${version_code}"
...
password_file="${TOOLCHAIN_DIR}/sub2api-signing-password"
keystore="${TOOLCHAIN_DIR}/sub2api-release.jks"
