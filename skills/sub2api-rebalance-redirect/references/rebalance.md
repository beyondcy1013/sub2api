...
This document is shared by all four rebalance skills (`safe`, `fast`, `aggressive`, `redirect`). It captures the root-cause analysis and the data fix that all plans share.
...
## The Two Data Fixes (shared by all four plans)
...
The two data fixes are the necessary precondition. The four plans differ in **how aggressively** to also break the existing sticky-session lock.
...
| D | `sub2api-rebalance-redirect` | `SET` all matching keys to target value | Breaks `previous_response_id` chains; sudden load on target | ~5 min |

All four plans apply the same two SQL fixes first. The difference is whether (and how) they also touch Redis.
